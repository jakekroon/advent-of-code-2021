...
func (caves *Graph) countPaths(src *Node, dest *Node, count int) int {
	if src == dest {
		return count + 1
	}
	...
	for _, nextNode := range src.next {
		visitable := !nextNode.small || nextNode.visited == 0 ||
			(nextNode.label != "start" && !(nextNode.small && nextNode.visited == 1))
		if visitable {
			nextNode.visited += 1
			count = caves.countPaths(nextNode, dest, count)
			nextNode.visited -= 1
		}
	}
	return count
...
func main() {
	...
	totalPaths := caveSystem.countPaths(caveSystem.nodes["start"], caveSystem.nodes["end"], 0)
